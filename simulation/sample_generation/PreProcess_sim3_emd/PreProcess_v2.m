% Compared to PreProcess.m, the phi_true is by importing from phi_true.mat
...
load phi_true.mat
[mRow, mCol] = size(phi_true);
mCols = 256;

additive_noise = 0.6*randn(mRow, mCol);
...
% % Need choose the same k2 for 1 and 2
% %%I_filtered = zeros(size(I_filtered));
% [I_filtered(:,:,1),~,~,k1_1,k2_1] = BSEMD_m( I_filtered(:,:,1),2 );
% [I_filtered(:,:,2),~,~,k1_2,k2_2] = BSEMD_m( I_filtered(:,:,2),2 );
% if(k2_1 ~= k2_2)
%     error('k2 are different!'); 
% end

% % % % % Define the mask
% xc = 128;  yc = 128;  % mask 1
xc = 173;  yc = 157;  % mask 2
% xc = 95;  yc = 165;   % mask 3
radius = round( 0.2*(min(mRow, mCol)/2) );

if yc-radius<1 || xc-radius<1 || xc+radius>mCol || yc+radius>mRow
...
% Need choose the same k2 for 1 and 2
%%I_filtered = zeros(size(I_filtered));
[Imask(:,:,1),~,~,k1_1,k2_1] = BSEMD_m( Imask(:,:,1),2 );
[Imask(:,:,2),~,~,k1_2,k2_2] = BSEMD_m( Imask(:,:,2),2 );
if(k2_1 ~= k2_2)
    error('k2 are different!'); 
end
...
save for_further.mat Io delta_true I_filtered xc yc radius zern in_vector k1_1 k1_2 k2_1 k2_2
